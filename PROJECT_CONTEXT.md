...
# 页面与本地 Express 接口一起启动
npm run dev
# → http://localhost:3001
```

本地 Express 已同时托管前端静态文件，不再需要第二个终端或 Python 静态服务器。
...
## 十三、v8 本地 MVP 更新（2026-09-22）

- 数据正式拆分为 `trainingPlans`（计划）和 `workoutLogs`（实际完成记录）。计划不再进入完成率、训练容量和加重判断。
- 新增“记录训练”页，用户填写实际重量、组数、次数、RPE 后才生成完成记录。
- AI 对话和个人设置修改今天训练类型时，会同步更新今日计划、首页、今日建议与个人设置，解决“对话说练胸但仍显示腿日”的问题。
- 训练分析维度三新增 `全部 / 胸 / 背 / 肩 / 手臂 / 核心 / 臀 / 腿` 单排筛选按钮。
- 接入开源 `body-muscles@1.0.0`，使用前后视、70+ 可交互肌肉区域替换旧的简化示意图。
- UI 切换为 FORGE AI 黑绿专业力量风；原生下拉选项增加深色配色，解决白底白字问题。
- `npm run dev` 现在同时提供前端页面和本地 API，统一访问 `http://localhost:3001`。
- 新本地存储键：`fitness-training-plans-v1`、`fitness-workout-logs-v1`；旧数据会自动迁移。
