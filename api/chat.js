const { callOpenAICompatible } = require('./_utils/llm-client');
...
module.exports = async function handler(req, res) {
  ...
  try {
    const { messages = [], profile = {}, workouts = [] } = req.body || {};
    ...
    const systemPrompt = [
      ...
      '',
      '【本周计划】',
      summarizeWeekPlan(workouts),
      '',
      '【预计算指标】',
      JSON.stringify(computeMetrics(workouts), null, 2),
      '',
      '【用户档案】',
    ...
    if (reply.functionCall) {
      res.json({ reply: null, mode: reply.mode, functionCall: reply.functionCall });
      return;
    }
